...
function displaySingle(entState)
{
	...
	// Mythology s
	let showStamina = entState.stamina;
	// Mythology e

	let healthSection = Engine.GetGUIObjectByName("healthSection");
	let staminaSection = Engine.GetGUIObjectByName("staminaSection");
	...
	// Mythology s
	// Stamina
	staminaSection.hidden = !showStamina;
	if (showStamina)
	{
		let unitStaminaBar = Engine.GetGUIObjectByName("staminaBar");
		let staminaSize = unitStaminaBar.size;
		staminaSize.rright = 100 * Math.max(0, Math.min(1, entState.stamina.points / entState.stamina.maxPoints));
		unitStaminaBar.size = staminaSize;
		Engine.GetGUIObjectByName("staminaStats").caption = sprintf(translate("%(hitpoints)s / %(maxHitpoints)s"), {
			"hitpoints": Math.ceil(entState.stamina.points),
			"maxHitpoints": Math.ceil(entState.stamina.maxPoints)
		});
		Engine.GetGUIObjectByName("stamina").tooltip = sprintf("Stamina: +%(regenRate)s points while idle", {
			"regenRate": Math.ceil(entState.stamina.regenRate)
		});
		staminaSection.size = sectionPosBottom.size;
	}
	// Mythology e
