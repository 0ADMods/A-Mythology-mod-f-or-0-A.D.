 ...
 */
function determineAction(x, y, fromMinimap, nClicks = 1)
{
	...
	if (preSelectedAction != ACTION_NONE)
	{
		for (var action of actions)
			...
			{
				if (g_UnitActions[action].clickCheck && !g_UnitActions[action].clickCheck(nClicks))
					continue;
...
function handleInputAfterGui(ev)
{
	...
	case INPUT_UNIT_POSITION_START:
		...
		case "mousebuttonup":
			...
			if (ev.button == SDL_BUTTON_RIGHT)
			{
				let action = determineAction(ev.x, ev.y, false, ev.clicks);
				if (action)
					return doAction(action, ev);
			}
			break;
