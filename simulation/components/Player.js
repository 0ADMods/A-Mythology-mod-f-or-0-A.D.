...
Player.prototype.Init = function()
{
	...
	let tradeCodes = Resources.GetTradableCodes();
	let quotient = Math.floor(20 / tradeCodes.length);
	let remainder = 20 % tradeCodes.length;
	...
	for (let i in tradeCodes)
	{
		let res = tradeCodes[i];
...
Player.prototype.SetDeity = function(name)
{
	...
	for (let deity of deityData)
		...
		{
			this.deity = deity;
			Engine.QueryInterface(this.entity, IID_TechnologyManager).UpdateAutoResearch();
		}
};

Player.prototype.GetDeity = function()
{
	return this.deity;
};
