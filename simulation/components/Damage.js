...
Damage.prototype.MissileHit = function(data, lateness)
{
	...
	if (data.isSplash)
	{
		this.CauseSplashDamage({
			...
			"knockBack": data.splashKnockBack
		});
	}
	...
	for (let ent of ents)
	{
		...
		// Did only the target and it's bonus changed here?
		let specificData = {
		...
		if (data.knockBack)
		{
			specificData.knockBack = data.knockBack;
			specificData.direction = data.direction;
		}
...
{
	let classes = Engine.QueryInterface(data.target, IID_Identity).GetClassesList();
	if (data.knockBack && !classes.every(cl => data.knockBack.AffectedClasses._string.split(/\s+/).indexOf(cl) == -1))
	{
		let strength = +data.knockBack.Strength;
		let direction = new Vector2D(0, 0);

		let cmpPosition = Engine.QueryInterface(data.target, IID_Position);
		let currentPos = cmpPosition.GetPosition2D();

		if (data.type.indexOf(".Splash") == -1)
		{
			if (data.type.indexOf("Melee") != -1)
			{
				let cmpAttackerPos = Engine.QueryInterface(data.attacker, IID_Position);
				if (cmpAttackerPos)
				{
					let attackerPos = cmpAttackerPos.GetPosition2D();
					direction = Vector2D.sub(currentPos, attackerPos).normalize();
				}
			}
			else
				direction = data.direction;
			...
			direction = data.direction || Vector2D.sub(currentPos, data.origin).normalize(); // predefined for linear splash

		let newPos = Vector2D.add(currentPos, Vector2D.mult(direction, strength));
		let inBounds = true;

		let size = Engine.QueryInterface(SYSTEM_ENTITY, IID_Terrain).GetMapSize();
		...
		if (Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager).GetLosCircular())
		{
			...
			if (pos.x < edgeSize || pos.z < edgeSize || pos.x > size - edgeSize || pos.z > size - edgeSize)
				inBounds = false;

		if (inBounds)
			cmpPosition.MoveTo(newPos.x, newPos.y);
	}
