...
// Checks an entity template to see if its technology requirements have been met and is deity is selected
TechnologyManager.prototype.CanProduce = function (templateName)
{
	...
	{
		if (template.Identity.RequiredTechnology && !this.IsTechnologyResearched(template.Identity.RequiredTechnology))
			return false;

		if (template.Identity.RequiredDeity &&
			template.Identity.RequiredDeity != Engine.QueryInterface(this.entity, IID_Player).GetDeity().name)
			return false;
	}
...
TechnologyManager.prototype.CheckTechnologyRequirements = function(reqs, civonly = false)
{
	...
	return reqs.some(req => {
		return Object.keys(req).every(type => {
			...
			case "deity":
				return cmpPlayer.GetDeity().name == req[type];
			}
			return false;
		});
	});
